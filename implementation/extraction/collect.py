...
class Collector(threading.Thread):
    ...
    def __init__(self, t_id):
        ...
        self.window_size = int(DATASET_SIZE / THREAD_SIZE)
        self.t_id=t_id
        self.low=t_id*self.window_size
        self.high=(t_id+1)*self.window_size
    ...
    def mine_binary_info(self):
        ...
        for index in self.curr_indices:
            ...
            for path in get_paths(func):
                self.final_results.add(refine_path_line(path[6:]))
            self.counter += 1
        print("Thread {}".format(self.t_id), ":", self.counter, "/", self.window_size)
        return True
